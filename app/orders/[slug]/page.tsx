...
import { OrderDetails } from "@/app/data/dummyTypes";
...
const OrderDetailsPage = ({
  ...
  useEffect(() => {
    if (orderId !== "") {
      axios
        .get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/shop/vendor/view-order-details/ORDER-${orderId}`,
          { headers: { Authorization: `Bearer ${token}` } },
        )
        .then((res) => {
          console.log("STEP 1", res.data.Data);
          if (res.status === 200) {
            const temp: OrderDetails[] = res.data.Data;

            temp.forEach((item) => {
              axios
                .get(
                  `${process.env.NEXT_PUBLIC_API_BASE_URL}/shop/vendor-product/${item.product}`,
                  { headers: { Authorization: `Bearer ${token}` } },
                )
                .then((res) => {
                  console.log("STEP 2", res.data.Data);
                  if (res.status === 200) {
                    const populatedTemp = temp.map((item) => {
                      return { ...item, product_details: res.data.Data };
                    });

                    console.log("POPULALTED!!!", populatedTemp);

                    setOrders(populatedTemp);
  ...
  if (isLoading) {
    ...
    if (orders && orders[0].product_details?.product_name) {
      return (
        <>
                        ...
                        <TableRow key={order.product}>
                          <TableCell>
                            <div className="flex items-center gap-3">
                              <Image
                                src={
                                  order.product_details?.items[0].product_image.slice(
                                    13,
                                  ) || ""
                                }
